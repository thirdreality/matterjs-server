...
| `<prefix>/<node>` | publish, retained | full merged state JSON, e.g. `{"state":"ON","brightness":200,"color_mode":"hs","color":{"hue":240,"saturation":100},"color_temp":250}`; sensors add `occupancy`, `illuminance` (lx), `temperature`, `humidity`, `battery`, `contact`; OTA-capable nodes add an `update` object (version, source, download progress); a property provided by several endpoints (e.g. a dual relay) gets the zigbee2mqtt `_<endpoint>` suffix (`state_1`), unique properties stay plain |

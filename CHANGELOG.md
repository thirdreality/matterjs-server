...
- Fix: (ThirdReality) The server no longer refuses to start for good after an unclean shutdown. A power cut leaves the storage lock files behind, and the upstream stale-lock check is just `kill(pid, 0)`, which carries nothing that survives a reboot and on Linux also resolves thread ids — so once anything at all inherits the recorded id, every start fails with `[storage-lock] Storage is locked by another process`. Before taking the locks the server now reclaims those whose recorded owner holds no file in this storage, which is decisive where a name check is not: a sibling such as the BLE proxy carries a matching name but no storage. Anything short of a definite "not using this storage" leaves the lock alone, since starting a second instance on live storage would be worse than not starting
...
- Fix: (ThirdReality) MQTT bridge clears retained topics it no longer publishes. A retained topic outlives the process that published it, so an entity dropped or renamed in a new version kept showing up in Home Assistant, and a node decommissioned while the bridge was down left its state and entities behind — both previously needed clearing on the broker by hand. After publishing the full picture the bridge now reconciles the retained discovery, state and availability topics the broker replays against what it just published. Only topics provably its own are touched: a discovery config must reference this bridge's topic prefix, and under its own prefix only an uncommissioned node's state or availability qualifies
- Feature: (ThirdReality) MQTT bridge completes the zigbee2mqtt light semantics: every z2m color form on `/set` (RGB, hex, HSL, HSB/HSV and their string spellings) with conversion to whichever representation the endpoint supports, relative control via `brightness`/`color_temp`/`hue`/`saturation` `_move` and `_step`, `/get` reading from the device instead of replaying the cache, and published color state synchronized across representations so a mode change never leaves a stale `color` or `color_temp` behind
